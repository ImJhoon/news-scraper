...
* **로직 흐름**:
  1. **마크다운 빌드**: `StringBuilder`를 사용하여 모바일/PC 모두에서 보기 좋은 **카드형 인용구(Blockquote)** 디자인의 마크다운 텍스트를 조립합니다. 추출된 썸네일 이미지는 HTML `<img>` 태그를 활용해 `width=300`으로 렌더링을 최적화합니다.
  2. **문자열 정제**: 제목이나 요약 내용에 큰따옴표(`"`)나 줄바꿈이 포함되면 GitHub API 통신 시 JSON 형식이 깨지는 심각한 오류가 발생할 수 있습니다. 이를 방어하기 위해 내부 헬퍼 메서드(`cleanTextForJson`)로 HTML 특수 태그 및 제어 문자를 완벽히 이스케이프 처리합니다.
