...
![뉴스 브리핑 이슈 화면](docs/images/preview.png)
...
* **✨ AI 3줄 요약**: 구글 최신 제미나이(Gemini 3.1 Flash Lite) API를 연동하여 기사 원문을 바탕으로 핵심 내용만 3줄로 깔끔하게 요약해 줍니다.
* **가독성 높은 레이아웃**: 추출된 데이터와 AI 요약본을 마크다운 썸네일 카드 형태로 가공하여 이슈 본문을 예쁘게 구성합니다.
...
* **API**: Naver Search API, Google Gemini API, GitHub REST API
...
2. 발급받은 API 키(`NAVER_CLIENT_ID`, `NAVER_CLIENT_SECRET`, `GEMINI_API_KEY`)를 입력합니다.
...
* **Secrets**: `NAVER_CLIENT_ID`, `NAVER_CLIENT_SECRET`, `GEMINI_API_KEY` (제미나이 키는 선택 사항이며 없으면 기본 요약 제공)
